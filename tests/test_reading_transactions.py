...
def test_successful_load_with_default_path(
    ...
    # Настройка итерации
    data = [
        {'date': '2024-01-15', 'description': 'Покупка продуктов',
         'amount': '1500.50', 'currency': 'RUB', 'category': 'Еда'},
        {'date': '2024-01-16', 'description': 'Оплата интернета',
         'amount': '890.00', 'currency': 'RUB', 'category': 'Услуги'}
    ]
...
@patch('builtins.open')
@patch('csv.DictReader')
def test_amount_conversion_to_float(
        mock_dict_reader: Mock,
        mock_open_file: Mock,
        mock_exists: Mock
) -> None:
...
def test_invalid_amount_handling(
    ...
    mock_reader.fieldnames = ['date', 'amount']
    data = [
        {'date': '2024-01-15', 'amount': 'invalid_amount'},
        {'date': '2024-01-16', 'amount': '890.00'}
    ]
    mock_reader.__iter__ = Mock(return_value=iter(data))
    mock_dict_reader.return_value = mock_reader
...
def test_string_stripping(
    ...
    data = [
        {' date ': ' 2024-01-15 ', ' description ': ' Покупка ', ' amount ': ' 1500.50 '}
    ]
    mock_reader.__iter__ = Mock(return_value=iter(data))
    mock_dict_reader.return_value = mock_reader
...
def test_empty_values_handling(
    ...
    mock_reader.fieldnames = ['date', 'description', 'amount']
    data = [
        {'date': '2024-01-15', 'description': '', 'amount': ''}
    ]
    mock_reader.__iter__ = Mock(return_value=iter(data))
    mock_dict_reader.return_value = mock_reader
...
def test_missing_amount_field(
    ...
    data = [
        {'date': '2024-01-15', 'description': 'Покупка', 'currency': 'RUB'}
    ]
    mock_reader.__iter__ = Mock(return_value=iter(data))
    mock_dict_reader.return_value = mock_reader
...
@patch('builtins.open')
@patch('csv.DictReader')
def test_numeric_values_other_than_amount(
        mock_dict_reader: Mock,
        mock_open_file: Mock,
        mock_exists: Mock
) -> None:
    ...
    data = [
        {'date': '2024-01-15', 'quantity': '5', 'price': '100.50', 'amount': '500.00'}
    ]
    mock_reader.__iter__ = Mock(return_value=iter(data))
    mock_dict_reader.return_value = mock_reader
...
@pytest.mark.parametrize("amount_value,expected_type", [
    ("100.50", float),
    ("invalid", str),
    ("", str),
    ("0", float),
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_load_with_xls_extension(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_load_with_xlsm_extension(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_custom_path_without_extension(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_custom_path_with_extension(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_file_found_without_extension_direct(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
...
@patch('os.path.exists')
@patch('pandas.read_excel')
def test_multiple_rows_loading(
        mock_read_excel: Mock,
        mock_exists: Mock
) -> None:
    ...
    test_data = {
        ...
        'amount': [i * 100 for i in range(1, 11)]
    }
    mock_df = pd.DataFrame(test_data)
    mock_read_excel.return_value = mock_df

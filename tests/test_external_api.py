...
def test_usd_conversion_success():
    ...
    # Настройка мока
    mock_response = Mock()
    mock_response.json.return_value = {"success": True, "rates": {"RUB": 75.50}}
...
def test_eur_conversion_success():
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"success": True, "rates": {"RUB": 90.25}}
    mock_response.raise_for_status = Mock()
...
def test_cache_prevents_duplicate_api_calls():
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"success": True, "rates": {"RUB": 75.00}}
    mock_response.raise_for_status = Mock()
...
def test_cache_separate_for_different_currencies():
    ...
    mock_response_usd.json.return_value = {"success": True, "rates": {"RUB": 75.00}}
    mock_response_usd.raise_for_status = Mock()

    mock_response_eur = Mock()
    mock_response_eur.json.return_value = {"success": True, "rates": {"RUB": 90.00}}
    mock_response_eur.raise_for_status = Mock()

    with patch.object(
        src.external_api.requests, "get", side_effect=[mock_response_usd, mock_response_eur]
    ) as mock_get:
        # Конвертация USD
...
def test_connection_error():
    ...
    with patch.object(
        src.external_api.requests, "get", side_effect=requests.exceptions.ConnectionError("Connection failed")
    ):
        transaction = {"amount": 100, "currency": "USD"}
